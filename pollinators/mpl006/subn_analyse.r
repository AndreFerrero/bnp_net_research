...
compute_d_obs <- function(edges, subn) {
  ...
  sub_e   <- edges %>% slice(df$row_index) %>% mutate(weight=df$weight)
  sub_pl  <- sub_e %>% group_by(plant)      %>% summarise(counts=sum(weight))
  sub_po  <- sub_e %>% group_by(pollinator) %>% summarise(counts=sum(weight))
  list(
...
for(p in percentages){
  ...
  ppc_hist <- ggplot(data.frame(density=d_ppc), aes(x=density)) +
                ...
                labs(title=paste0(pct_label,"% PPC density"),
